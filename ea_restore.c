...
void restore_zval (zval * zv TSRMLS_DC)
{
    switch (zv->type & ~IS_CONSTANT_INDEX) {
        case IS_CONSTANT:
        case IS_STRING:
            if (zv->value.str.val == NULL || zv->value.str.val == empty_string
                    || zv->value.str.len == 0) {
                zv->value.str.val = empty_string;
                return;
            } else {
        ...
        case IS_OBJECT:
            {
                ...
                if (class_name != NULL) {
                    ...
                    if (zend_hash_find(CG (class_table), 
                                (void *) class_name, name_len + 1, 
                                (void **) &ce) != SUCCESS) {
                        char *lowercase_name =
                            estrndup (INCOMPLETE_CLASS, sizeof (INCOMPLETE_CLASS));
                        ...
                        if (zend_hash_find(CG (class_table), lowercase_name, 
                            ...
                            zend_error (E_ERROR, "EACCELERATOR can't restore object's class \"%s\"",
                                    class_name);
                        } else {
                            efree (lowercase_name);
                            zv->value.obj.ce = ce;
                ...
                if (zv->value.obj.properties != NULL) {
                    zv->value.obj.properties =
                        restore_zval_hash (NULL, zv->value.obj.properties);
                ...
                if (incomplete_class && class_name != NULL) {
                    ...
                    zend_hash_update (Z_OBJPROP_P (zv), MAGIC_MEMBER,
                            sizeof (MAGIC_MEMBER), &val, sizeof (val),
                            NULL);
                }
#endif
                return;
        ...
        eaccelerator_op_array * from TSRMLS_DC)
{
    ...
    if (from->type == ZEND_INTERNAL_FUNCTION) {
        ...
        if (ZendOptimizer) {
            zend_llist_apply_with_argument (&zend_extensions,
                    (llist_apply_with_arg_func_t) call_op_array_ctor_handler,
                    to TSRMLS_CC);
        }
    }
    to->type = from->type;
    ...
    if (from->scope_name != NULL) {
        ...
        if (zend_hash_find
                (CG (class_table), (void *) from_scope_lc, from->scope_name_len + 1,
        ...
        if (MMCG (class_entry)) {
            ...
            for (p = MMCG (class_entry)->parent; p; p = p->parent) {
                ...
                if (zend_hash_find
                        (&p->function_table, fname_lc, fname_len + 1,
                         (void **) &function) == SUCCESS) {
                    ea_debug_pad (EA_DEBUG TSRMLS_CC);
                    ea_debug_printf (EA_DEBUG,
                            "[%d]                                   '%s' has '%s' of scope '%s'\n",
                            getpid (), p->name, fname_lc,
    ...
    if (from->type == ZEND_INTERNAL_FUNCTION) {
        ...
        if (ce != NULL &&
                ce->parent != NULL && zend_hash_find (&ce->parent->function_table,
#ifdef ZEND_ENGINE_2
                    fname_lc, fname_len + 1,
#else
                    to->function_name,
                    strlen (to->function_name) +
                    1,
#endif
                    (void **) &function) ==
                SUCCESS && function->type == ZEND_INTERNAL_FUNCTION) {
            ea_debug_pad (EA_DEBUG TSRMLS_CC);
            ea_debug_printf (EA_DEBUG,
                    "[%d]                                       found in function table\n",
                    getpid ());
            ((zend_internal_function *) (to))->handler =
                ((zend_internal_function *) function)->handler;
        } else {
...
static zend_class_entry *restore_class_entry (zend_class_entry * to,
        ...
        from TSRMLS_DC)
{
    ...
    if (to->num_interfaces > 0) {
        to->interfaces =
            (zend_class_entry **) emalloc (sizeof (zend_class_entry *) *
                                           to->num_interfaces);
        // XXX:
        //
        // should find out class entry. what the hell !!!
        memset (to->interfaces, 0,
                sizeof (zend_class_entry *) * to->num_interfaces);
    } else {
        to->interfaces = NULL;
    }
    ...
    if (from->parent != NULL) {
        ...
        if (zend_hash_find
                (CG (class_table), (void *) name_lc, name_len + 1,
                 (void **) &to->parent) != SUCCESS)
#else
            if (zend_hash_find
                    (CG (class_table), (void *) from->parent, name_len + 1,
                     (void **) &to->parent) != SUCCESS)
#endif
            {
                ea_debug_error ("[%d] EACCELERATOR can't restore parent class "
                        "\"%s\" of class \"%s\"\n", getpid (),
                        (char *) from->parent, to->name);
                to->parent = NULL;
            } else {
#ifdef ZEND_ENGINE_2
                /*
...
void restore_function (mm_fc_entry * p TSRMLS_DC)
{
    ...
    if (restore_op_array (&op_array, (eaccelerator_op_array *) p->fc TSRMLS_CC)
            != NULL) {
        if (zend_hash_add
...
#ifdef ZEND_ENGINE_2
    ce = restore_class_entry (NULL,
            (eaccelerator_class_entry *) p->fc TSRMLS_CC);
    if (ce != NULL)
#else
        if (restore_class_entry (&ce, (eaccelerator_class_entry *) p->fc TSRMLS_CC)
                != NULL)
#endif
        {
#ifdef ZEND_ENGINE_2
