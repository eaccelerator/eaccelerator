...
void calc_zval (zval * zv TSRMLS_DC)
{
    ...
    case IS_OBJECT:
        ...
        if (zv->value.obj.ce != NULL) {
            ...
            calc_string (zv->value.obj.ce->name,
        ...
        for (i = 0; i < from->num_args; i++) {
            if (from->arg_info[i].name) {
                calc_string (from->arg_info[i].name,
                             from->arg_info[i].name_len + 1 TSRMLS_CC);
            }
            if (from->arg_info[i].class_name) {
                calc_string (from->arg_info[i].class_name,
                             from->arg_info[i].class_name_len + 1 TSRMLS_CC);
            }
        }
    }
#else
    if (from->arg_types != NULL) {
        calc_string ((char *) from->arg_types,
                     (from->arg_types[0] + 1) * sizeof (zend_uchar) TSRMLS_CC);
    }
#endif
    if (from->function_name != NULL) {
        calc_string (from->function_name,
                     strlen (from->function_name) + 1 TSRMLS_CC);
    }
#ifdef ZEND_ENGINE_2
    if (from->scope != NULL) {
...
void calc_class_entry (zend_class_entry * from TSRMLS_DC)
{
    ...
    if (from->parent != NULL && from->parent->name) {
        calc_string (from->parent->name,
                     from->parent->name_length + 1 TSRMLS_CC);
    }
#ifdef ZEND_ENGINE_2
#if 0
...
static void store_hash_int (HashTable * target, HashTable * source,
                            Bucket * start,
                            store_bucket_t copy_bucket TSRMLS_DC)
{
    Bucket *p, *np, *prev_p;
...
void store_zval (zval * zv TSRMLS_DC)
{
    ...
    case IS_STRING:
        if (zv->value.str.val == NULL ||
            ...
            zv->value.str.val =
                store_string (zv->value.str.val,
                              zv->value.str.len + 1 TSRMLS_CC);
        }
        break;
    case IS_ARRAY:
    case IS_CONSTANT_ARRAY:
...
#ifndef ZEND_ENGINE_2
        if (zv->value.obj.ce != NULL) {
            char *s = store_string (zv->value.obj.ce->name,
                                    zv->value.obj.ce->name_length +
                                    1 TSRMLS_CC);
...
eaccelerator_op_array *store_op_array (zend_op_array * from TSRMLS_DC)
{
    ...
    if (from->num_args > 0) {
        ...
        for (i = 0; i < from->num_args; i++) {
            if (from->arg_info[i].name) {
                to->arg_info[i].name =
                    store_string (from->arg_info[i].name,
                                  from->arg_info[i].name_len + 1 TSRMLS_CC);
                to->arg_info[i].name_len = from->arg_info[i].name_len;
            }
            if (from->arg_info[i].class_name) {
                to->arg_info[i].class_name =
                    store_string (from->arg_info[i].class_name,
                                  from->arg_info[i].class_name_len +
                                  1 TSRMLS_CC);
                to->arg_info[i].class_name_len =
                    from->arg_info[i].class_name_len;
            }
            to->arg_info[i].allow_null = from->arg_info[i].allow_null;
            to->arg_info[i].pass_by_reference =
                from->arg_info[i].pass_by_reference;
            to->arg_info[i].return_reference =
                from->arg_info[i].return_reference;
        }
    }
    ...
    if (from->arg_types != NULL) {
        to->arg_types = (unsigned char *)
            store_string ((char *) from->arg_types,
                          (from->arg_types[0] +
                           1) * sizeof (zend_uchar) TSRMLS_CC);
    }
#endif
    if (from->function_name != NULL) {
        /*
         ...
         */
        to->function_name =
            store_string (from->function_name,
                          strlen (from->function_name) + 1 TSRMLS_CC);
        // XXX: revert
        ...
        while (q != NULL) {
            if (*(zend_class_entry **) q->pData == from->scope) {
                ...
                ea_debug_printf (EA_DEBUG,
                                 ...
                                 getpid (),
                                 from->scope->name ? from->scope->name : "NULL",
                                 q->arKey, to->scope_name);
                break;
            }
            q = q->pListNext;
        }
...
*/
            ...
            case ZEND_JMP:
                opline->op1.u.jmp_addr =
                    to->opcodes + (opline->op1.u.jmp_addr - from->opcodes);
                break;
            case ZEND_JMPZ:
            case ZEND_JMPNZ:
            case ZEND_JMPZ_EX:
            case ZEND_JMPNZ_EX:
                opline->op2.u.jmp_addr =
                    to->opcodes + (opline->op2.u.jmp_addr - from->opcodes);
                break;
            }
#endif
        }
    ...
    if (from->doc_comment != NULL) {
        to->doc_comment =
            store_string (from->doc_comment,
                          from->doc_comment_len + 1 TSRMLS_CC);
    }
#endif
    return to;
}
...
#ifdef ZEND_ENGINE_2
static zend_property_info *store_property_info (zend_property_info *
                                                from TSRMLS_DC)
{
...
*/
    ...
    if (from->filename != NULL) {
        to->filename =
            store_string (from->filename,
                          strlen (from->filename) + 1 TSRMLS_CC);
    }
    if (from->doc_comment != NULL) {
        to->doc_comment =
            store_string (from->doc_comment,
    ...
    store_zval_hash (&to->default_properties, &from->default_properties);
#endif
    store_hash (&to->function_table, &from->function_table,
